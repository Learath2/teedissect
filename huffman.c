...
typedef struct
{
	...
	int m_Frequency;
} CHuffmanConstructNode;

void Setbits_r(struct huff_ctx *huff, struct CNode *pNode, int Bits, unsigned Depth)
{
	if(pNode->m_aLeafs[1] != 0xffff)
		Setbits_r(huff, &huff->m_aNodes[pNode->m_aLeafs[1]], Bits|(1<<Depth), Depth+1);
	if(pNode->m_aLeafs[0] != 0xffff)
		Setbits_r(huff, &huff->m_aNodes[pNode->m_aLeafs[0]], Bits, Depth+1);
...
static void ConstructTree(struct huff_ctx *huff, const unsigned *pFrequencies)
{
	...
	{
		huff->m_aNodes[i].m_NumBits = 0xFFFFFFFF;
		huff->m_aNodes[i].m_Symbol = i;
		huff->m_aNodes[i].m_aLeafs[0] = 0xffff;
		huff->m_aNodes[i].m_aLeafs[1] = 0xffff;
	...
	huff->m_NumNodes = HUFFMAN_MAX_SYMBOLS;
	...
	while(NumNodesLeft > 1)
	{
		...
		huff->m_aNodes[huff->m_NumNodes].m_NumBits = 0;
		huff->m_aNodes[huff->m_NumNodes].m_aLeafs[0] = apNodesLeft[NumNodesLeft-1]->m_NodeId;
		huff->m_aNodes[huff->m_NumNodes].m_aLeafs[1] = apNodesLeft[NumNodesLeft-2]->m_NodeId;
		apNodesLeft[NumNodesLeft-2]->m_NodeId = huff->m_NumNodes;
	...
	// set start node
	huff->m_pStartNode = &huff->m_aNodes[huff->m_NumNodes-1];

	// build symbol bits
	Setbits_r(huff, huff->m_pStartNode, 0, 0);
}
...
void huffman_init(struct huff_ctx *huff)
{
	...
	// construct the tree
	ConstructTree(huff, gs_aFreqTable);
	...
	for(int i = 0; i < HUFFMAN_LUTSIZE; i++)
	{
		unsigned Bits = i;
		int k;
		struct CNode *pNode = huff->m_pStartNode;
		for(k = 0; k < HUFFMAN_LUTBITS; k++)
		{
			pNode = &huff->m_aNodes[pNode->m_aLeafs[Bits&1]];
			Bits >>= 1;
...
#define HUFFMAN_MACRO_LOADSYMBOL(Sym) \
	Bits |= huff->m_aNodes[Sym].m_Bits << Bitcount; \
	Bitcount += huff->m_aNodes[Sym].m_NumBits;
...
int huffman_decompress(struct huff_ctx *huff, const void *pInput, int InputSize, void *pOutput, int OutputSize)
{
	...
	struct CNode *pEof = &huff->m_aNodes[HUFFMAN_EOF_SYMBOL];
	struct CNode *pNode = 0;
	...
	while(1)
	{
		...
		if(Bitcount >= HUFFMAN_LUTBITS)
			pNode = huff->m_apDecodeLut[Bits&HUFFMAN_LUTMASK];
